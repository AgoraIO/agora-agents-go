...
func TestOpenAIEmitsInlineToolsAlongsideMcpServers(t *testing.T) {
	...
	if config["mcp_servers"].([]map[string]interface{})[0]["transport"] != "streamable_http" {
		t.Fatalf("MCP transport was not normalized: %#v", config["mcp_servers"])
	}
}

func TestLlmVendorOptionsExposeToolsLikeMcpServers(t *testing.T) {
