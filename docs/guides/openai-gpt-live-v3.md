...
live := vendors.NewOpenAIGPTLive(vendors.OpenAIGPTLiveOptions{
    ...
    McpServers: []map[string]interface{}{{"name": "lookup", "endpoint": "https://tools.example/mcp"}},
})
// Pass live to your agent's WithMllm method.
// Enable advanced_features.enable_tools with WithTools(true).
```

Set MCP servers on `OpenAIGPTLiveOptions`; they serialize as `mllm.mcp_servers`. Enable `advanced_features.enable_tools` with the existing tools builder. To advertise graph tools to GPT Live, also set `ToolEnabled` to true.
